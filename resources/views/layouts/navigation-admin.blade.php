...
<aside class="hidden lg:flex lg:flex-col w-64 bg-[#003E60] text-white min-h-screen">
  ...
  <nav class="px-0 py-4 space-y-1">
    <a href="{{ route('admin.dashboard') }}"
       ...
       {{ request()->routeIs('admin.*') ? 'bg-[var(--mb-primary)] text-white' : 'text-white/80 hover:bg-white/10' }} rounded-none">
      <!-- icône Dashboard améliorée -->
      <svg class="w-5 h-5 flex-shrink-0" viewBox="0 0 24 24" fill="{{ request()->routeIs('admin.*') ? 'currentColor' : 'none' }}" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" xmlns="http://www.w3.org/2000/svg">
